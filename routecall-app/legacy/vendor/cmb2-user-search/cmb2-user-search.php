<?php
/*
  orignial code from https://github.com/Mte90/CMB2-User-Search-field
 */
...
function cmb2_user_search_render_field( $field, $escaped_value, $object_id, $object_type, $field_type ) {
	...
	if ( !empty( $field->escaped_value ) ) {
		...
		foreach ( $list as $user_id ) {
			...
			echo '<li data-id="' . trim( $user_id ) . '">' . $name;
	...
	if ( !is_admin() ) {
		...
		do_action( 'cmb2_user_search_add_find_users_div' );
	}
	            ...
	            handleSelected: function (checked) {
	              ...
	              if (newids.indexOf(',') !== -1) {
	                ...
	                if ($('.cmb-type-user-search-text.cmb2-id-<?php echo str_replace( '_', '-', sanitize_html_class( $field->args( 'id' ) ) ) ?> ul li[data-id="' + newids + '"]').length === 0) {
	                  $('.cmb-type-user-search-text.cmb2-id-<?php echo str_replace( '_', '-', sanitize_html_class( $field->args( 'id' ) ) ) ?> ul').append('<li data-id="' + newids + '">' + this.$checkedLabel[0] + '<div title="<?php _e( 'Remove' ) ?>" style="color: #999;margin: -0.1em 0 0 2px; cursor: pointer;" class="cmb-user-search-remove dashicons dashicons-no"></div></li>');
	                }
	              }
...
add_action( 'cmb2_user_search_find_users_div', 'cmb2_user_search_add_find_users_div' );
...
function cmb2_user_search_format_name( $user_id ){
	// TODO make common routine to format display of associated USER details
}
 ...
 * @return string
 */
function cmb2_user_search_display_name( $search_columns ) {
	$search_columns[] = 'display_name';
	return $search_columns;
}
add_filter( 'user_search_columns', 'cmb2_user_search_display_name' );
