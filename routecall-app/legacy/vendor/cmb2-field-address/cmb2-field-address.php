...
if ( ! class_exists( 'CMB2_Field_Address' ) ) {
  ...
  class CMB2_Field_Address {
    ...
    public static function render_address( $field, $field_escaped_value, $field_object_id, $field_object_type, $field_type_object ) {
      ...
      $show_label = isset($field->args['show_label']) && $field->args['show_label'] == false ? $field->args['show_label'] : true;

			?>
			<div>
        <?php if($show_label) : ?><label for="<?= $field_type_object->_id( '_location', false ); ?>"><?= esc_html(  'Location' ); ?></label><?php endif; ?>
				<p><?= $field_type_object->input( [
					'type'	=> 'text',
          ...
          'placeholder' => 'Location',
			...
			<div>
        <?php if($show_label) : ?><label for="<?= $field_type_object->_id( '_address_1', false ); ?>"><?= esc_html(  'Address 1' ); ?></label><?php endif; ?>
				<p><?= $field_type_object->input( [
					'type'	=> 'text',
          ...
          'placeholder' => 'Address',
			...
			<div>
        <?php if($show_label) : ?><label for="<?= $field_type_object->_id( '_address_2', false ); ?>'"><?= esc_html(  'Address 2' ); ?></label><?php endif; ?>
				<p><?= $field_type_object->input( [
					'type'	=> 'text',
          ...
          'placeholder' => 'Address 2',
			...
			<div style="overflow: hidden;">
				<div class="alignleft">
          <?php if($show_label) : ?><label for="<?= $field_type_object->_id( '_city', false ); ?>'"><?= esc_html( 'City' ); ?></label><?php endif; ?>
					<p><?= $field_type_object->input( [
						'type'	=> 'text',
        ...
        ?>
				<div class="alignleft">
          <?php if($show_label) : ?><label for="<?= $field_type_object->_id( '_state', false ); ?>'"><?= esc_html(  $state_label ); ?></label><?php endif; ?>

						<p><?= $field_type_object->select( [
				...
				<div class="alignleft">
          <?php if($show_label) : ?><label for="<?= $field_type_object->_id( '_zip', false ); ?>'"><?= esc_html( 'Zip' ); ?></label><?php endif; ?>
					<p><?= $field_type_object->input( [
						'type'  => 'number',
						'class' => 'cmb_text_small',
